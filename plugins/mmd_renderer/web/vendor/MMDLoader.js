 ...
 */
   ...
   */
		...
		_normalizeTexturePath( filePath ) {

			return String( filePath || '' ).replace( /\\/g, '/' );

		}

		_encodeTexturePath( filePath ) {

			const normalized = this._normalizeTexturePath( filePath );
			if ( /^(?:[a-z][a-z0-9+.-]*:)?\/\//i.test( normalized ) || /^data:/i.test( normalized ) ) {
		...
		_resourcePathLastSegment() {

			const path = String( this.resourcePath || '' ).replace( /\/+$/, '' );
		...
		_resolveTextureFullPath( filePath ) {

			const normalized = this._normalizeTexturePath( filePath );
			const parts = normalized.split( '/' ).filter( function ( part ) { return part.length > 0; } );
			if ( parts.length > 1 && parts[ 0 ].toLowerCase() === this._resourcePathLastSegment() ) {

				const trimmed = parts.slice( 1 ).join( '/' );
				const fullPath = this.resourcePath + this._encodeTexturePath( trimmed );
				console.warn( 'THREE.MMDLoader: dropped duplicated texture directory ' + JSON.stringify( {
					filePath: filePath,
					fullPath: fullPath
				} ) );
				return fullPath;
			...
			return this.resourcePath + this._encodeTexturePath( normalized );
		...
		_loadTexture( filePath, textures, params, onProgress, onError ) {
			...
			if ( params.isDefaultToonTexture === true ) {
				...
				fullPath = this._resolveTextureFullPath( filePath );
			...
			console.log( 'THREE.MMDLoader: texture load start ' + JSON.stringify( {
				filePath: filePath,
				fullPath: fullPath,
				isToonTexture: params.isToonTexture === true
			} ) );
			...
			const texture = loader.load( fullPath, function ( t ) {
				...
				console.log( 'THREE.MMDLoader: texture load OK ' + JSON.stringify( {
					filePath: filePath,
					fullPath: fullPath,
					width: t.image && t.image.width ? t.image.width : null,
					height: t.image && t.image.height ? t.image.height : null
				} ) );

			}, onProgress, function ( err ) {

				const message = err && err.message ? err.message : String( err );
				console.error( 'THREE.MMDLoader: texture load FAILED ' + JSON.stringify( {
					filePath: filePath,
					fullPath: fullPath,
					error: message
				} ) );
			...
			texture.userData.sakuraFullPath = fullPath;
